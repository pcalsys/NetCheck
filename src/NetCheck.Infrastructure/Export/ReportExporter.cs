...
public sealed class ReportExporter : IReportExporter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var redactedAdapters = report.Network.Adapters
            .Select(adapter => adapter with { MacAddress = "Redacted" })
            .ToArray();
        var redactedPrimary = report.Network.PrimaryAdapter is null
            ...
            : redactedAdapters.FirstOrDefault(adapter => adapter.Id == report.Network.PrimaryAdapter.Id)
              ?? report.Network.PrimaryAdapter with { MacAddress = "Redacted" };
        var exportNetwork = report.Network with
        {
            MachineName = includeComputerName ? report.Network.MachineName : "Redacted",
        ...
        var content = extension switch
        {
            ...
            _ => throw new NotSupportedException("NetCheck can export HTML, JSON, or plain-text reports.")
        };
    ...
    private static string CreateText(DiagnosticReport report)
    {
        var builder = new StringBuilder();
        builder.AppendLine("NETCHECK DIAGNOSTIC REPORT");
        builder.AppendLine(new string('=', 72));
        builder.AppendLine($"Report ID: {report.Id}");
        builder.AppendLine($"Completed: {report.CompletedAtUtc.ToLocalTime():yyyy-MM-dd HH:mm:ss zzz}");
        builder.AppendLine($"Computer: {report.Network.MachineName}");
        builder.AppendLine($"Outcome: {report.Diagnosis.Outcome}");
        builder.AppendLine($"Diagnosis: {report.Diagnosis.Headline}");
        builder.AppendLine(report.Diagnosis.Summary);
        builder.AppendLine();
        builder.AppendLine("NETWORK");
        builder.AppendLine($"Adapter: {report.Network.PrimaryAdapter?.Name ?? "Not available"}");
        builder.AppendLine($"IP address: {report.Network.PrimaryIpAddress}");
        builder.AppendLine($"Gateway: {report.Network.PrimaryGateway}");
        ...
        {
            builder.AppendLine();
            builder.AppendLine($"[{check.Status.ToString().ToUpperInvariant()}] {check.Title}");
    ...
    private static string CreateHtml(DiagnosticReport report)
    {
        ...
        builder.AppendLine($"<title>NetCheck report {report.Id}</title>");
        ...
        builder.AppendLine($"<section class=\"hero\"><div class=\"brand\">NETCHECK</div><h1>{Encode(report.Diagnosis.Headline)}</h1><p>{Encode(report.Diagnosis.Summary)}</p><p class=\"muted\">Completed {Encode(report.CompletedAtUtc.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss zzz"))} · Report {report.Id}</p></section>");
        builder.AppendLine("<section class=\"grid\">");
        AppendMetric(builder, "Computer", report.Network.MachineName);
        AppendMetric(builder, "Adapter", report.Network.PrimaryAdapter?.Name ?? "Not available");
        AppendMetric(builder, "IP address", report.Network.PrimaryIpAddress);
        AppendMetric(builder, "Gateway", report.Network.PrimaryGateway);
        builder.AppendLine("</section><section class=\"checks\">");

        foreach (var check in report.Checks)
        {
            builder.AppendLine($"<article class=\"card check\"><div class=\"status {check.Status}\">{Encode(check.Status.ToString().ToUpperInvariant())}</div><h3>{Encode(check.Title)}</h3><p>{Encode(check.Summary)}</p>");
            if (!string.IsNullOrWhiteSpace(check.Detail))
            {
            ...
            {
                builder.AppendLine("<strong>Recommended actions</strong><ul>");
                foreach (var recommendation in check.Recommendations)
                {
